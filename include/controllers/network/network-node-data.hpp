...
public:
    ...
    NetworkNodeData(net::address remote, uint64_t timestamp) :
    ...
    NetworkNodeData(const id_t id,
        ...
        net::address remote, uint64_t timestamp)
    ...
    NetworkNodeData(const id_t id,
        ...
        std::function<void(uint8_t*, const uint8_t*, size_t, uint64_t)>&& hasher_tcp,
            net::address remote, uint64_t timestamp)
            : _id(id), _verifier(std::move(verifier)),
            _hasher_udp(std::move(hasher_udp)),
            _hasher_tcp(std::move(hasher_tcp)),
     ...
     *
     */
    const std::function<bool(const uint8_t*, const uint8_t*, size_t, uint64_t)>& Verifier() const {
        return _verifier;
    }
     ...
     *
     */
    const std::function<void(uint8_t*, const uint8_t*, size_t, uint64_t)>& Hasher_UDP() const {
        return _hasher_udp;
    }
     ...
     *
     */
    const std::function<void(uint8_t*, const uint8_t*, size_t, uint64_t)>& Hasher_TCP() const {
        return _hasher_tcp;
    }
    ...
    memory::StreamAllocator<4096>* UDPReliableBuffer() {
        return udp_reliable_buffer.get();
    }

    const net::address& GetRemoteAddress() const { return _addr; }

private:
    std::unique_ptr<memory::StreamAllocator<4096>> udp_reliable_buffer;
    const net::address _addr;
    const std::function<bool(const uint8_t*, const uint8_t*, size_t, uint64_t)> _verifier;
    const std::function<void(uint8_t*, const uint8_t*, size_t, uint64_t)> _hasher_udp;
    const std::function<void(uint8_t*, const uint8_t*, size_t, uint64_t)> _hasher_tcp;
