...
template<>
float Initialize<Component::OxygenRate>(bool& result, const std::vector<Property> &properties) {
    id_t entity_id;
...
bool Initialize<Component::Movable>(bool& result, const std::vector<Property> &properties) {
    ...
    if(movable) {
        ...
        auto act = component::GetContainer<Component::Interactable>(entity_id);
        component::Get<Component::Interactable>(act)->AddAction(Action::IA_MOVE);
    }
    return movable;
}
